...
def create_pdf_bookmarks(file_list, file_name_output):
    """Создаем и добавляем закладки и meta данные в файл"""
...
def merge_files_directories(dir_name, file_name_output, prog):
    ...
    # Читаем содержимое директории dir_name, проверяем наличие pdf-файлов,
    # если нет завершаем программу.
    file_list = [
        os.path.join(dir_name, files)
        for files in
        sorted(filter(lambda file_ext: file_ext.endswith(
            '.pdf'), os.listdir(dir_name)))
    ]
...
def merge_files_list(file_list, file_name_output, prog):
    ...
    file_list = list(
        filter(lambda file_ext: file_ext.endswith('.pdf'), file_list))
...
def create_parser(args):
    """Разбираем аргументы args, командной строки. Возвращаем аргументы командной строки, args,
    без имени программы."""

    prog = args[0]
    args = args[1:]
    parser = argparse.ArgumentParser(formatter_class=argparse.RawDescriptionHelpFormatter,
                                     description='Converts pdf files into one multi-page document.',
                                     epilog='description:'
                                            '\n  -f, --file\tspecifies the file(s) to be merged'
                                            '\n  -d, --dir\tspecifies the directory in which to '
                                            'merge the files'
                                            '\n  -n, --name\tspecifies '
                                            'the name of the output file resulting from'
                                            '\n\t\tconcatenating source files\n\nexamples:'
                                            '\n  pdfmark -d DIR_NAME -n FILENAME_OUTPUT'
                                            '\n\tMerge all files from the directory DIR_NAME into '
                                            'a file FILENAME_OUTPUT.'
                                            '\n  pdfmark -f FILE(s) -n FILENAME_OUTPUT'
                                            '\n\tMerge the specified files into file '
                                            'FILENAME_OUTPUT.')

    # Добавляем необходимые нам ключи.
    parser.add_argument('-f', '--file', nargs='+', type=str,
                        help='set the name of the file(s)')
    parser.add_argument('-d', '--dir', nargs=1, type=str,
                        help='set the name of directory')
    parser.add_argument('-n', '--name', nargs=1, type=str,
                        help='set the name of the output file')

    # Если аргументов нет, выводим справку.
    if len(args) == 0:
        sys.exit(f'usage: {os.path.basename(prog)} [-h]\n{os.path.basename(prog)}: error: no '
                 f'arguments specified: {os.path.basename(prog)}')
...
def main(args):
    ...
    if options.dir and options.file or not options.file and not options.dir and options.name:
        sys.exit(
            f'usage: {os.path.basename(args[0])} [-h | --help].\n{os.path.basename(args[0])}: '
            f'error: parameters are incorrect')

    # Проверяем ключи и запускаем соответствующие действия.
    else:

        if options.name:
            file_name_output = os.path.splitext(options.name[0])[0] + ".pdf" if not \
                os.path.splitext(options.name[0])[1] else options.name[0]

            if options.file:
                merge_files_list(options.file, file_name_output,
                                 os.path.basename(args[0]))

            if options.dir:

                if os.path.isdir(options.dir[0]):
                    merge_files_directories(options.dir[0], file_name_output,
                                            os.path.basename(args[0]))

                else:
                    sys.exit(
                        f'{os.path.basename(args[0])}: error: Directory {options.dir[0]} does not '
                        f'exist.')

        else:
            sys.exit(
                f'usage: {os.path.basename(args[0])} [-h | --help].\n{os.path.basename(args[0])}: '
                f'error: output file name not specified.')
